...
// State Machine
//--------------------------------------
//   timeout and ack greater or equal to n/2+1
	...
	// 2. check GTID
	{
		...
		if greater {
			...
			if r.mysql.Promotable() {
				// localcommit cases: but raft is binlog + pos should use gtid
				// 1. I am promotable: I am alive and GTID greater than you
				//if r.mysql.CheckGTID(&rsp.GTID, &req.GTID) {
				//	rsp.RetCode = model.ErrorInvalidLocalGTID
				//	return rsp
				//}
